...
async function criar(req, res) {
  ...
  if (!Number(estoque_id)) {
    return fail(res, 400, "estoque_id é obrigatório");
  }
  if (!(await estoqueService.findById(Number(estoque_id)))) {
    return fail(res, 404, "Estoque não encontrado");
  }
...
function normalizeProductPayload(raw, index) {
  ...
  if (!codigo_barras || !String(codigo_barras).trim()) {
    throw Object.assign(new Error(`${label}: codigo_barras é obrigatório`), { status: 400 });
  }
  if (!nome || !String(nome).trim()) {
    throw Object.assign(new Error(`${label}: nome é obrigatório`), { status: 400 });
  }
  if (categoria_id == null || !Number(categoria_id)) {
    throw Object.assign(new Error(`${label}: categoria_id é obrigatório`), { status: 400 });
  }
  if (preco_venda == null || isNaN(Number(preco_venda)) || Number(preco_venda) < 0) {
    throw Object.assign(new Error(`${label}: preco_venda inválido`), { status: 400 });
  }
  if (!Number(estoque_id)) {
    throw Object.assign(new Error(`${label}: estoque_id é obrigatório`), { status: 400 });
  }
  if (estoque_atual != null && (isNaN(Number(estoque_atual)) || Number(estoque_atual) < 0)) {
    throw Object.assign(new Error(`${label}: estoque_atual inválido`), { status: 400 });
  }
  if (estoque_minimo != null && (isNaN(Number(estoque_minimo)) || Number(estoque_minimo) < 0)) {
    throw Object.assign(new Error(`${label}: estoque_minimo inválido`), { status: 400 });
  }
  ...
  try {
    ...
    for (const estoqueId of estoqueIds) {
      if (!(await estoqueService.findById(estoqueId))) {
        return fail(res, 404, `Estoque não encontrado: ${estoqueId}`);
      }
    }
...
async function atualizarLote(req, res) {
  ...
  if (!id) return fail(res, 400, "Produto inválido");
  if (!loteId) return fail(res, 400, "Lote inválido");
...
async function atualizar(req, res) {
  ...
  if (!id) {
    return fail(res, 400, "Produto inválido");
  }
  if (!codigo_barras || !nome) {
    return fail(res, 400, "codigo_barras e nome são obrigatórios");
  }
